...
options应包含所有必需参数如hostname，port,method等等,例如
{
    ...
    method: 'POST',
    headers: {
        'Content-Type': 'application/x-web-form-urlencoded',
        'Authorization': '',
    },
};
 */
...
//注册接口
_fns.reg = function(ctx) {
    var co = $co(function * () {

        var phone = ctx.query.phone || ctx.request.body.phone;
        if (!phone ) throw Error('请输入手机号.');
        var phoneCode = ctx.query.messageNum || ctx.request.body.messageNum;
        if (!phoneCode ) throw Error('请输入验证码.');
        var pw = ctx.query.pw || ctx.request.body.pw;
        if (!pw ) throw Error('请输入密码.');

        var path = '/start/api/regByPhone';
...
*!/
        ...
        console.log(msg);
        if (msg.code != 1) throw Error('获取用户信息失败，请稍后再试:' + msg.text);
        console.log('fns里的数据'+msg.data);
        return msg.data;
    });
    return co;
};*/
//获取Id
_fns.getUid = function(ctx) {
    var co = $co(function * () {
